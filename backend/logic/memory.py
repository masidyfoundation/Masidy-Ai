...
# Use service role key so backend bypasses RLS entirely
# Falls back to SUPABASE_KEY (anon) if service key not set
...
def get_or_create_user(external_id: str) -> dict:
    ...
    if client:
        try:
            ...
            print(f"[Warning] Supabase error in get_or_create_user: {e}. Using local fallback.")
...
def add_message(conversation_id: str, role: str, content: str) -> dict:
    """Appends a message to a conversation. Uses Supabase with local fallback."""
    client = get_supabase()

    if client:
        try:
            new_msg = client.table("messages").insert({
                "conversation_id": conversation_id,
                "role": role,
                "content": content
            }).execute()
            return new_msg.data[0] if new_msg.data else {
                "id": str(uuid.uuid4()),
                "conversation_id": conversation_id,
                "role": role,
                "content": content
            ...
            print(f"[Warning] Supabase error in add_message: {e}. Using local fallback.")
...
def get_history(conversation_id: str, limit: int = 20) -> list:
    ...
    if client:
        try:
            ...
            print(f"[Warning] Supabase error in get_history: {e}. Using local fallback.")
